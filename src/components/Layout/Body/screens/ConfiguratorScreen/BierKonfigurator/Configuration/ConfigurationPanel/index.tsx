...
const ConfigurationPanel = () => {
    ...
    return (
        <section className={classes.mh_20}>
            ...
            <ConfigurationSteps active={active} changeActive={changeActive} navPoints={navPoints}
                                setNavPoints={setNavPoints} changeActiveMarker={changeActiveMarker}
                                hideHoverText={hideHoverText} showHoverText={showHoverText}/>
            ...
            <div className={classes.flex} style={{width: '100%', padding: '0px 4px'}}>
                <div style={{width: '100%'}}>{(active !== 0) ? <button className={`${classes.btn} ${classes.btn_left}`}
                                                                       onClick={() => onClick(active - 1)}>Zurück</button> : null}</div>
                <div style={{width: '100%'}}>{(active !== 3) ? <button className={`${classes.btn} ${classes.btn_right}`}
                                                                       onClick={() => onClick(active + 1)}>Weiter</button> : null}</div>
            </div>
        </section>
    )
};
