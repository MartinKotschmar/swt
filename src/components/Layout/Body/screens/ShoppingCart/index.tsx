...
const ShoppingCart = (props: any) => {
    ...
    if (orders2[0] && orders2[0].bottleSize !== undefined) {
        ...
        let categories = sortedItems[0].map((a: any) => a.categories).map((e: any) => e.filter((e: any) => e !== 'Komponente' && e !== 'Zutat'));
        let arr: any = [];
        ...
        finalPrice = sortedItems[0].reduce((a: number, b: any) => a + (b.price.amount || 0), 0);
            ...
            <div className={classes.container}>
                ...
                <section className={`${classes.flex} ${classes.fd_column}`}>
                    <div className={`${classes.cart} ${classes.flex} ${classes.fd_column}`}>
                        ...
                        <div className={`${classes.product_title}`}>Text: {(order.bottleText)? order.bottleText: <span style={{color:'#aaa', fontStyle:'italic'}}>[ohne Text]</span>}</div>
                    </div>
                </section>
                <hr/>
